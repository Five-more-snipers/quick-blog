...
<div class="mt-6 bg-white shadow-sm rounded-lg divide-y"> 
    @foreach ($blogs as $blog)
        <div class="p-6 flex space-x-2" wire:key="{{ $blog->id }}">
            ...
            <div class="flex-1">
                <div class="flex justify-between items-center">
                    ...
                    @if ($blog->user->is(auth()->user()))
                        <x-dropdown>
                            ...
                            <x-slot name="content">
                                <x-dropdown-link wire:click="edit({{ $blog->id }})">
                                    {{ __('Edit') }}
                                </x-dropdown-link>
                                <x-dropdown-link wire:click="delete({{ $blog->id }})" wire:confirm="Are you sure to delete this chirp?"> 
                                    {{ __('Delete') }}
                                </x-dropdown-link> 
                            </x-slot>
                        </x-dropdown>
                ...
                @if ($blog->is($editing)) 
                    <livewire:blogs.edit :blog="$blog" :key="$blog->id" />
                @else
